...
main() {
  ...
  local tools_dir=$(echo "${KLIBIO}/tool")
  local tools_archives="${tools_dir}/archives"
  local installer_dir="${tools_dir}/eclipse-installer"

  mkdir -p ${tools_archives}

  local download_url="https://download.eclipse.org/oomph/products/latest/eclipse-inst-jre-${oomph_suffix}"
  local output_file="eclipse-inst-jre-${oomph_suffix}"
