...
if [[ $OSTYPE == msys ]]; then
  ...
  export java_home_suffix=/bin                # java
...
if [[ "$OSTYPE" == darwin* ]]; then
  ...
  export java_home_suffix=/Contents/MacOS/libjli.dylib  # java
  if [[ "$(uname -a)" == *"arm"* ]]; then
...
export java_bin="${KLIBIO}/java/ee/JAVA17${java_home_suffix:-}"          # java
...
function assure_ec_global_properties() {
  if [[ ! -f ~/.ec_global.properties ]]; then
    echo "configure your .ec_global.properties inside your HOME folder"
    echo "download file from here https://pages.git.i.mercedes-benz.com/smaragd/doc/idefix/template/ec_global.properties"
